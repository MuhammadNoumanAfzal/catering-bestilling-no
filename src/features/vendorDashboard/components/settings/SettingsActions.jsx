...
export default function SettingsActions({
  ...
  return (
    <section className="space-y-3">
      <h2 className="type-h4 text-[#191919]">{saveHeading === "Save changes" ? t("settings.saveHeading") : saveHeading}</h2>
      <div className="flex flex-col items-stretch gap-3 sm:flex-row sm:justify-end">
        ...
        >
          {resetLabel === "Reset" ? t("settings.reset") : resetLabel}
        </button>
        <button
          type="button"
          onClick={onSave}
        ...
        >
          {isSaving
            ? (savingLabel === "Saving..." ? t("settings.saving") : savingLabel)
            : (saveLabel === "Save changes" ? t("settings.save") : saveLabel)}
        </button>
      </div>
    </section>
  );
