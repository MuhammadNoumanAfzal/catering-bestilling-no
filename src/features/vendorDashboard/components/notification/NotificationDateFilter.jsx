    ...
    <div className="relative" ref={menuRef}>
      ...
      >
        <span className="truncate">
          {getNotificationDateFilterLabel(selectedRange, customDateRange, nt)}
        </span>
        <FiChevronDown
        ...
        <div className="absolute right-0 top-[calc(100%+10px)] z-20 min-w-[290px] rounded-[18px] border border-[#ead8cb] bg-white p-2 shadow-[0_18px_40px_rgba(31,24,19,0.12)]">
          {NOTIFICATION_DATE_OPTIONS.map((option) => {
            ...
            return (
              ...
              >
                <span>{nt(`date.${option.value === "all-time" ? "allTime" : option.value === "last-month" ? "lastMonth" : option.value === "last-3-months" ? "last3Months" : option.value === "last-6-months" ? "last6Months" : option.value === "this-year" ? "thisYear" : "customDate"}`)}</span>
                {isSelected ? (
                  <span className="text-[10px] font-semibold uppercase">{nt("date.active")}</span>
                ) : null}
              </button>
            );
          })}
