...
function SkeletonRow({ columns = 7 }) {
  return (
    <div
...
export default function OrdersLoadingState({ rows = 5, columns = 7 }) {
  return (
    <section className="overflow-hidden rounded-[16px] border border-[#ddd6cf] bg-white shadow-[0_6px_16px_rgba(53,34,20,0.05)]">
      <div className="relative bg-[radial-gradient(circle_at_top,rgba(233,122,63,0.10),transparent_46%),linear-gradient(180deg,#fffdfa_0%,#ffffff_100%)] px-5 py-6">
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div className="space-y-2">
            ...
            <div>
              <h2 className="text-[20px] font-extrabold text-[#211711]">Loading order activity</h2>
              <p className="mt-1 max-w-[520px] text-[14px] leading-6 text-[#7d6f65]">
                Gathering your order, event, delivery, and payment records.
              </p>
            </div>
          </div>
      ...
      <div className="bg-white">
        ...
        >
          {Array.from({ length: columns }).map((_, index) => (
            <SkeletonLine key={index} className={`h-3.5 ${index === columns - 1 ? "w-12" : index % 3 === 0 ? "w-20" : "w-24"}`} />
          ))}
        </div>
