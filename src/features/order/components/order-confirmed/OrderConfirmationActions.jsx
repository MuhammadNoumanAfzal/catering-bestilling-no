...
export default function OrderConfirmationActions({ canModify, modifyButtonLabel = "Modify Order", modifyDisabled = false, onModify }) {
  const { t } = useTranslation();
  return (
    <div className="mt-4">
      <div className={`grid grid-cols-2 gap-2 ${canModify ? "sm:grid-cols-3" : ""}`}>
        <Link to="/vendor-dashboard/orders" className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-[#cf6e38] px-3 py-2 text-[13px] font-semibold text-white hover:bg-[#bb602d]">
